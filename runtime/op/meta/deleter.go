...
func (d *Deleter) nextDeletion() (zbuf.Puller, error) {
	for {
		...
		scanner, object, err := newScanner(d.octx.Context, d.octx.Zctx, d.pool, d.unmarshaler, nil, nil, d.pruner, d.filter, &progress, &vals[0])
		if err != nil {
			return nil, err
		}
...
func (d *Deleter) hasDeletes(val *zed.Value) (bool, error) {
	scanner, object, err := newScanner(d.octx.Context, d.octx.Zctx, d.pool, d.unmarshaler, nil, nil, d.pruner, d.filter, d.progress, val)
	if err != nil {
		return false, err
	}
	var count uint64
